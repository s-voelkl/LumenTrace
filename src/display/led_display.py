...
class LedDisplay:
    ...
    def __init__(
        self, real_strips: dict[int, PixelStrip], virtual_strips: list[VirtualLedStrip]
    ):  # type: ignore
        """
        ...
        # Map each VirtualLedStrip to its offset within the logical lane array.
        self.__vs_lane_offsets: dict[int, int] = {}
        lane_lengths: dict[int, int] = {}

        for vs in virtual_strips:
            lid = vs.lane.lane_id
            offset = lane_lengths.get(lid, 0)
            self.__vs_lane_offsets[id(vs)] = offset
            lane_lengths[lid] = offset + vs.length

        for lid, length in lane_lengths.items():
            self.virtual_arrays[lid] = [(0, 0, 0)] * length
        ...
        # - A logical lane can be composed of multiple disjoint virtual strips
        #   (e.g., for multiple intersections). The offsets mapping ensures
        #   that the correct section of the virtual buffer is pushed to each
        #   physical segment.
    ...
    def set_lane_pixel_by_ratio(
        self, lane: Lane, ratio: float, color: tuple[int, int, int]
    ):
    ...
    def render(self):
        ...
        for vs in self.virtual_strips:
            ...
            if strip:
                ...
                offset = self.__vs_lane_offsets.get(id(vs), 0)
                for i in range(vs.length):
                    if offset + i >= len(arr):
                        break
                    color = arr[offset + i]
