...
def test_play_single_sound(mock_sf_read, mock_sd_outputstream):
    ...
    sound_id = manager.play(
        GameSound.ENGINE, volume=80.0, left_volume=100.0, right_volume=50.0
    )
    mock_sf_read.assert_called_once_with(GameSound.ENGINE.path, dtype="float32")
...
def test_play_multiple_sounds(mock_sf_read):
    ...
    id1 = manager.play(GameSound.ENGINE, pitch=1.0)
    id2 = manager.play(GameSound.STARTUP, pitch=1.5)
...
def test_update_sound(mock_sf_read):
    manager = SoundManager()
    sound_id = manager.play(GameSound.ENGINE, volume=50.0)
...
def test_audio_callback_mixing(mock_sf_read):
    ...
    manager._audio_cache[GameSound.ENGINE.path] = data1
    manager._audio_cache[GameSound.STARTUP.path] = data2

    # Play sound 1 full right
    id1 = manager.play(GameSound.ENGINE, left_volume=0.0, right_volume=100.0)

    # Play sound 2 full left
    id2 = manager.play(GameSound.STARTUP, left_volume=100.0, right_volume=0.0)
    ...
    # Note: Because of smoothing (_EASING_FACTOR) combined with numpy vectorized blocks, the first frames will interpolate
    # towards the target. For the sake of validation, we manually run _audio_callback
    # and verify the matrix has mixed outputs.
    ...
    # After the new vectorized callback, the output will mix both arrays according to volumes.
    # We just ensure left speaker contains data from sound 2, and right from sound 1.
